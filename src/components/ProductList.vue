    ...
    <div v-if="showCreateForm" class="card p-3 mb-3">
      ...
      <div class="form-group mb-2">
        <select v-model="newProduct.productStatusId" class="form-control">
          <option v-for="status in productStatuses" :key="status.id" :value="status.id">{{ status.productStatus }}
          </option>
        </select>
      </div>
...
<script>
import { fetchAllProducts, createProduct, updateProduct, deleteProduct, fetchAllProductTypes,
fetchAllProductStatuses } from '../services/api';
